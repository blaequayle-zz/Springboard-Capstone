...
for(i in 2011:2016){
  for(j in 1:12){
    searchurl <- paste0(paste0("https://data.police.uk/api/crimes-street/all-crime?poly=51.514,-0.226:51.484,-0.225:51.487,-0.105&date=",i),paste0("-",j))
    cr <- GET(searchurl)
...
#Using looping index to obtain 2017 datasets
crime2017 <- list()
k <- 1
for(i in 1:3){
    searchurl <- paste0(paste0("https://data.police.uk/api/crimes-street/all-crime?poly=51.514,-0.226:51.484,-0.225:51.487,-0.105&date=2017-",i))
    cr <- GET(searchurl)
    cc <- content(cr, as="text",encoding = "UTF-8")
    ct <- cc %>% fromJSON()
    crime2017[[k]] <- ct
